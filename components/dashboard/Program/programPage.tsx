...
export default function CoursesPage() {
  ...
  const applyPreference = (user) => {
    ...
    const profileData =
      profile?.role === "counsellor" ? user?.profile : allProfile?.profile;
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      ...
      setCountries(
        countriesData.map((c: any) => ({ label: c.name, value: c.code })),
      );
              ...
              >
                ...
                <div className="flex items-center justify-between p-6">
                  ...
                  <div className="flex gap-4">
                    <button
                      onClick={() =>
                        downloadPDF(selectedProgram, selectedFields)
                      }
                    ...
                    <button
                      onClick={() =>
                        downloadExcel(selectedProgram, selectedFields)
                      }
                      className="bg-orange-500 px-6 py-3 rounded-xl text-white hover:scale-105 transition"
                    >
                      Download as Excel
            ...
            <div className="grid grid-cols-1 gap-5">
              {loading ? (
                ...
                courses.map((course, index) => {
                  ...
                  const openIntakes = intakeData.filter(
                    (item) => !item.isClosed,
                  );
                  const closedIntakes = intakeData.filter(
                    (item) => item.isClosed,
                  );
                  ...
                  const fallbackClosed = metaInfo?.IntakesClosed
                    ? metaInfo.IntakesClosed.split(",").map((item) => {
                        const [month, year, open, closed, remark] =
                          item.split(":::");
                        return {
                          month: month.trim(),
                  ...
                  const fallbackClosedMonths = fallbackClosed?.map(
                    (item) => item.month,
                  );
                  ...
                  return (
                    ...
                    <div
                      key={course._id}
                      className="fade-in-up h-full flex flex-col rounded-lg hover:scale-103 transition-transform duration-200"
                    >
                      <div
                        className={`relative w-full p-4 md:p-5 transition-all duration-200 hover:shadow-md hover:shadow-orange-500/20 h-full flex flex-col border border-orange-500 rounded-xl ${
                        ...
                        <div className="flex flex-col md:flex-row gap-4 md:gap-14 min-w-0 w-full mb-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm font-medium text-gray-600 mb-1 flex items-center gap-1 break-words">
                              <svg
                                className="w-5 h-5 md:w-6 md:h-6 text-orange-400 flex-shrink-0"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                              ...
                              <div className="flex flex-col px-2">
                                ...
                                <p className="font-bold text-emerald-600 text-sm md:text-base">
                                  {metaInfo?.AverageScholarship
                                    ? `${metaInfo.AverageScholarship} ${course?.currency}`
                                    : "N/A"}
                                </p>
                              </div>
                            </div>
                              ...
                              <div className="flex flex-col px-2 last:pr-0">
                                ...
                                <p className="font-bold text-gray-700 text-sm md:text-base">
                                  {metaInfo?.initialDeposit
                                    ? `${metaInfo.initialDeposit} ${course?.currency}`
                                    : "0 MYR"}
                                </p>
                              </div>
                            </div>
