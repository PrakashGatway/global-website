...
//       let res = await axiosInstance.put(`/applications/documents/${id}/${docId}`, fd, {
//         headers: { "Content-Type": "multipart/form-data" },
//       });
    ...
    >
      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          {/* File upload — only shown when NOT editing and category is not form */}
          {!isEditing && docCategory !== "form" && (
            <div>
...
export default function ApplicationDetailPage() {
  ...
  // Fetch courses for the university when application loads
  useEffect(() => {
    const fetchCourses = async () => {
      const code = application?.course?.university?.code;
      if (!code) return;
  ...
  // ── Document Upload ────────────────────────────────────────────────────────
  // Matches backend: PUT /applications/documents/:applicationId/:documentId
  // Sends: file (multipart), docType (to skip docUrl for forms), name, answer
  ...
  // Also uses PUT /applications/documents/:applicationId/:documentId
  // Sends updated metadata fields as FormData (no file)
