...
const Step1APSApplied = ({ data, currentStepId, apiData }) => {
  const stepData = apiData?.steps?.find(s => s.id === 1) || data.steps[0];
  const overviewDetails = stepData?.sections?.overview?.details || data.steps[0].sections.overview.details;
  
  return (
    <>
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{stepData?.sections?.overview?.title || data.steps[0].sections.overview.title}</h3>
          <span className="text-xs text-gray-500">Updated: {stepData?.sections?.overview?.updated || data.steps[0].sections.overview.updated}</span>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-2">{data.steps[0].sections.whatIsThis.title}</h3>
        <p className="text-xs text-gray-500 mb-4">{data.steps[0].sections.whatIsThis.description}</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {data.steps[0].sections.whatIsThis.items.map((item, idx) => {
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {data.steps[0].sections.preparationCards.map((card, idx) => {
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[0].sections.documents.title}</h3>
          <button className="text-[#f56e45] text-xs font-bold bg-orange-50 px-3 py-1 rounded">View All Documents →</button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
          {data.steps[0].sections.documents.columns.map((column, colIdx) => (
            <div key={colIdx}>
              {column.map((doc, idx) => {
                const statusColor = {
...
const Step2APSApproval = ({ data, currentStepId, apiData }) => {
  const stepData = apiData?.steps?.find(s => s.id === 2) || data.steps[1];
  
  return (
    <>
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[1].sections.approvalDetails.title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[1].sections.approvalDetails.items.map((item, idx) => (
        ...
        <div className="mt-6 pt-6 flex justify-center">
          <div className="bg-white border border-green-200 rounded-lg p-4 w-full max-w-[280px] flex items-center gap-4 shadow-sm">
            <div className="flex-1">
              ...
              <h5 className="font-bold text-sm text-gray-800">{data.steps[1].sections.approvalDetails.certificate.title}</h5>
              <p className="text-[10px] text-green-600 font-medium">{data.steps[1].sections.approvalDetails.certificate.status}</p>
            </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[1].sections.whatHappensNext.title}</h3>
        <p className="text-xs text-gray-500 mb-6">{data.steps[1].sections.whatHappensNext.description}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {data.steps[1].sections.whatHappensNext.steps.map((step, idx) => {
        ...
        <div className="mt-6 flex justify-center">
          <button className="flex items-center gap-2 bg-[#f56e45] hover:bg-[#f56e45] text-white text-sm font-medium px-6 py-2.5 rounded-lg transition-colors shadow-sm">
            {data.steps[1].sections.whatHappensNext.actionButton} <ArrowRight size={16} />
          </button>
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[1].sections.documents.title}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100">
                {data.steps[1].sections.documents.columns.map((col, idx) => (
                  <th key={idx} className="pb-2 text-[10px] font-semibold text-gray-500 text-left">{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.steps[1].sections.documents.rows.map((row, idx) => (
        ...
        <div className="mt-4 flex justify-center">
          <button className="flex items-center gap-2 border border-orange-200 bg-orange-50 text-[#f56e45] text-xs font-medium px-4 py-2 rounded-lg">
            <Eye size={14} /> {data.steps[1].sections.documents.buttonText}
          </button>
        </div>
      </div>
    </>
...
const Step3VisaApplication = ({ data, currentStepId, apiData }) => {
  const stepData = apiData?.steps?.find(s => s.id === 3) || data.steps[2];
  
  return (
    <>
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.applicationInfo.title}</h3>
          <span className="text-xs text-[#f56e45] cursor-pointer hover:underline flex items-center gap-1">
          </span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[2].sections.applicationInfo.items.map((item, idx) => (
            <DetailItem key={idx} item={item} />
          ))}
        </div>
        {stepData?.progress && (
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.personalInfo.title}</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[2].sections.personalInfo.items.map((item, idx) => (
            <div key={idx}>
              <p className="text-[10px] text-gray-500">{item.label}</p>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.familyInfo.title}</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="border border-gray-100 rounded-lg p-4">
            <h5 className="text-xs font-bold text-gray-700 mb-3">Father's Details</h5>
            <div className="space-y-2">
              <div><span className="text-[10px] text-gray-500">Name</span><p className="font-medium text-sm">{data.steps[2].sections.familyInfo.father.name}</p></div>
              <div><span className="text-[10px] text-gray-500">Occupation</span><p className="font-medium text-sm">{data.steps[2].sections.familyInfo.father.occupation}</p></div>
              <div><span className="text-[10px] text-gray-500">Phone</span><p className="font-medium text-sm">{data.steps[2].sections.familyInfo.father.phone}</p></div>
            </div>
          </div>
          <div className="border border-gray-100 rounded-lg p-4">
            <h5 className="text-xs font-bold text-gray-700 mb-3">Mother's Details</h5>
            <div className="space-y-2">
              <div><span className="text-[10px] text-gray-500">Name</span><p className="font-medium text-sm">{data.steps[2].sections.familyInfo.mother.name}</p></div>
              <div><span className="text-[10px] text-gray-500">Occupation</span><p className="font-medium text-sm">{data.steps[2].sections.familyInfo.mother.occupation}</p></div>
              <div><span className="text-[10px] text-gray-500">Phone</span><p className="font-medium text-sm text-gray-400">{data.steps[2].sections.familyInfo.mother.phone || "N/A"}</p></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.travelInfo.title}</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[2].sections.travelInfo.items.map((item, idx) => (
            <div key={idx}>
              <p className="text-[10px] text-gray-500">{item.label}</p>
              <p className="text-sm font-medium text-gray-800">{item.value}</p>
            </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.academicInfo.title}</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[2].sections.academicInfo.items.map((item, idx) => (
            <div key={idx}>
              <p className="text-[10px] text-gray-500">{item.label}</p>
              <p className="text-sm font-medium text-gray-800">{item.value}</p>
            </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.financialInfo.title}</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[2].sections.financialInfo.items.map((item, idx) => (
            <div key={idx}>
              <p className="text-[10px] text-gray-500">{item.label}</p>
              <p className="text-sm font-medium text-gray-800">{item.value}</p>
            </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[2].sections.documents.title}</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody>
              {data.steps[2].sections.documents.rows.map((row, idx) => {
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[2].sections.declarations.title}</h3>
        <div className="flex items-start gap-3 mb-4">
          <input type="checkbox" className="mt-1 w-4 h-4 rounded border-gray-300 text-[#f56e45]" />
          <p className="text-xs text-gray-600">{data.steps[2].sections.declarations.checkboxText}</p>
        </div>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-6 text-xs">
            <div><p className="text-[10px] text-gray-500">Date</p><p className="font-medium">{data.steps[2].sections.declarations.date}</p></div>
            <div><p className="text-[10px] text-gray-500">Applicant Name</p><p className="font-medium">{data.steps[2].sections.declarations.name}</p></div>
          </div>
...
const Step4Biometrics = ({ data, currentStepId, apiData }) => {
  const stepData = apiData?.steps?.find(s => s.id === 4) || data.steps[3];
  
  return (
    <>
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[3].sections.appointmentDetails.title}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-4 mb-6">
          {data.steps[3].sections.appointmentDetails.items.map((item, idx) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {data.steps[3].sections.appointmentDetails.infoCards.map((item, idx) => (
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-2">{data.steps[3].sections.whatIsBiometrics.title}</h3>
        <p className="text-xs text-gray-500 mb-3">{data.steps[3].sections.whatIsBiometrics.description}</p>
        <button className="flex items-center gap-2 text-orange-600 text-xs font-medium hover:underline">
          <PlayCircle size={16} /> {data.steps[3].sections.whatIsBiometrics.watchButton}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-800 mb-3">{data.steps[3].sections.documentsToCarry.title}</h3>
          <div className="space-y-1 mb-3">
            {data.steps[3].sections.documentsToCarry.items.map((text, idx) => (
          ...
          <p className="text-[10px] text-gray-400 italic">{data.steps[3].sections.documentsToCarry.note}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-800 mb-3">{data.steps[3].sections.biometricsProcess.title}</h3>
          <div className="space-y-1">
            {data.steps[3].sections.biometricsProcess.steps.map((step, idx) => (
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-3">{data.steps[3].sections.beforeYouGo.title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          {data.steps[3].sections.beforeYouGo.items.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 py-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-gray-800">{data.steps[3].sections.feesDetails.title}</h3>
            <button className="flex items-center gap-1 text-xs text-orange-600"><Download size={12} /> Download Receipt</button>
          </div>
          <div className="space-y-1">
            {data.steps[3].sections.feesDetails.items.map((item, idx) => (
          ...
          <div className="mt-3 text-[10px] text-gray-400 border-t pt-2">
            <p>Payment Date: {data.steps[3].sections.feesDetails.paymentDate}</p>
            <p>Payment Mode: {data.steps[3].sections.feesDetails.paymentMode}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-800 mb-4">{data.steps[3].sections.appointmentHistory.title}</h3>
          <div className="space-y-1">
            {data.steps[3].sections.appointmentHistory.items.map((item, idx) => (
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[3].sections.afterBiometrics.title}</h3>
        <div className="flex flex-wrap justify-between gap-2">
          {data.steps[3].sections.afterBiometrics.steps.map((step, idx) => {
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[3].sections.declaration.title}</h3>
        <div className="flex items-start gap-3 mb-4">
          <input type="checkbox" className="mt-1 w-4 h-4 rounded border-gray-300 text-orange-600" />
          <p className="text-xs text-gray-600">{data.steps[3].sections.declaration.checkboxText}</p>
        </div>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-6 text-xs">
            <div><p className="text-[10px] text-gray-500">Date</p><p className="font-medium">{data.steps[3].sections.declaration.date}</p></div>
            <div><p className="text-[10px] text-gray-500">Applicant Name</p><p className="font-medium">{data.steps[3].sections.declaration.name}</p></div>
          </div>
...
const Step5VisaDecision = ({ data, currentStepId, apiData }) => {
  const stepData = apiData?.steps?.find(s => s.id === 5) || data.steps[4];
  
  return (
    <>
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[4].sections.applicationDetails.title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[4].sections.applicationDetails.items.map((item, idx) => (
            <DetailItem key={idx} item={item} />
          ))}
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[4].sections.currentStatus.title}</h3>
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 bg-orange-50 p-4 rounded-lg border border-orange-100 flex flex-col items-center justify-center text-center">
            ...
            <h4 className="font-bold text-sm text-orange-800">{data.steps[4].sections.currentStatus.status}</h4>
            <p className="text-xs text-gray-600 mt-1">{data.steps[4].sections.currentStatus.description}</p>
          </div>
          <div className="flex-1 space-y-2">
            {data.steps[4].sections.currentStatus.details.map((item, idx) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-800 mb-4">{data.steps[4].sections.decisionTimeline.title}</h3>
          <div className="space-y-1">
            {data.steps[4].sections.decisionTimeline.items.map((item, idx) => {
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-800 mb-4">{data.steps[4].sections.whatHappensNext.title}</h3>
          <div className="space-y-1">
            {data.steps[4].sections.whatHappensNext.steps.map((step, idx) => (
          ...
          <div className="mt-4 p-3 bg-orange-50 border border-orange-100 rounded-lg">
            <h5 className="text-xs font-bold text-orange-800 flex items-center gap-1">
              <AlertCircle size={12} /> {data.steps[4].sections.whatHappensNext.importantNote.title}
            </h5>
            <ul className="mt-1 space-y-1">
              {data.steps[4].sections.whatHappensNext.importantNote.items.map((text, idx) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-gray-800">{data.steps[4].sections.submittedDocuments.title}</h3>
            <span className="text-xs text-orange-600 cursor-pointer hover:underline">View All Documents →</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <tbody>
                {data.steps[4].sections.submittedDocuments.rows.map((row, idx) => (
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-gray-800">{data.steps[4].sections.embassyUpdate.title}</h3>
            <span className="text-xs text-orange-600 cursor-pointer hover:underline">View All</span>
          </div>
          <div className="space-y-1">
            {data.steps[4].sections.embassyUpdate.items.map((item, idx) => (
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[4].sections.declaration.title}</h3>
        <div className="flex items-start gap-3 mb-4">
          <input type="checkbox" className="mt-1 w-4 h-4 rounded border-gray-300 text-orange-600" />
          <p className="text-xs text-gray-600">{data.steps[4].sections.declaration.checkboxText}</p>
        </div>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-6 text-xs">
            <div><p className="text-[10px] text-gray-500">Date</p><p className="font-medium">{data.steps[4].sections.declaration.date}</p></div>
            <div><p className="text-[10px] text-gray-500">Applicant Name</p><p className="font-medium">{data.steps[4].sections.declaration.name}</p></div>
          </div>
          <button className="border border-gray-200 text-gray-600 text-xs font-medium px-4 py-2 rounded hover:bg-gray-50">Save & Print</button>
        </div>
      </div>
...
const Step6VisaApproved = ({ data, currentStepId, apiData }) => {
  const stepData = apiData?.steps?.find(s => s.id === 6) || data.steps[5];
  
  return (
    <>
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[5].sections.visaApprovalDetails.title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[5].sections.visaApprovalDetails.items.map((item, idx) => (
            <DetailItem key={idx} item={item} />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center justify-center text-center">
          ...
          <h3 className="font-bold text-lg text-gray-800">{data.steps[5].sections.congratulations.title}</h3>
          <p className="text-xs text-gray-600 mt-2">{data.steps[5].sections.congratulations.message}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-800 mb-4">{data.steps[5].sections.whatHappensNext.title}</h3>
          <div className="flex flex-wrap justify-between gap-2">
            {data.steps[5].sections.whatHappensNext.steps.map((step, idx) => {
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[5].sections.visaDetails.title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {data.steps[5].sections.visaDetails.items.map((item, idx) => (
            <div key={idx}>
              <p className="text-[10px] text-gray-500">{item.label}</p>
              <p className="text-sm font-medium text-gray-800">{item.value}</p>
            </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[5].sections.passportCollection.title}</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-3 mb-3">
          {data.steps[5].sections.passportCollection.items.map((item, idx) => (
            <div key={idx}>
              <p className="text-[10px] text-gray-500">{item.label}</p>
              <p className="text-sm font-medium text-gray-800">{item.value}</p>
            </div>
        ...
        <div className="p-3 bg-orange-50 border border-orange-100 rounded-lg flex items-start gap-2">
          ...
          <p className="text-xs text-orange-700">{data.steps[5].sections.passportCollection.note}</p>
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-800">{data.steps[5].sections.documentsSummary.title}</h3>
          <span className="text-xs text-orange-600 cursor-pointer hover:underline">View All Documents →</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <tbody>
              {data.steps[5].sections.documentsSummary.rows.map((row, idx) => (
                <DocumentRow key={idx} row={row} showSize={false} />
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-gray-800 mb-1">{data.steps[5].sections.embassyMessage.title}</h3>
            <p className="text-sm text-gray-600">{data.steps[5].sections.embassyMessage.message}</p>
          </div>
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h3 className="font-bold text-gray-800 mb-4">{data.steps[5].sections.importantInfo.title}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {data.steps[5].sections.importantInfo.items.map((item, idx) => (
...
export default function VisaJourneyPage() {
  ...
  const data = apiData || staticVisaJourneyData;

  // Get current step data - merge API step data with static data for complete sections
  const currentStepData = data.steps?.find(step => step.id === currentStep) || staticVisaJourneyData.steps.find(step => step.id === currentStep) || staticVisaJourneyData.steps[0];
  ...
  // Determine if we should use static steps for display (for complete sections)
  const staticStepData = staticVisaJourneyData.steps.find(step => step.id === currentStep);

  // Render the appropriate step component
  const renderStepContent = () => {
    const stepProps = {
      data: staticVisaJourneyData,
    ...
    switch (currentStep) {
      case 1:
        return <Step1APSApplied {...stepProps} />;
      case 2:
        return <Step2APSApproval {...stepProps} />;
      case 3:
        return <Step3VisaApplication {...stepProps} />;
      case 4:
        return <Step4Biometrics {...stepProps} />;
      case 5:
        return <Step5VisaDecision {...stepProps} />;
      case 6:
        return <Step6VisaApproved {...stepProps} />;
      default:
        return <Step1APSApplied {...stepProps} />;
    }
  };
  ...
  // Get steps for progress bar - use API steps if available, otherwise static
  const progressSteps = data.steps && data.steps.length > 0 ? data.steps : staticVisaJourneyData.steps;
          ...
          <div className={`${getBannerStyles()} rounded-xl p-4 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border`}>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {bannerData.subtitle && <p className="text-xs mt-0.5">{bannerData.subtitle}</p>}
              </div>
            </div>
            {bannerData.action && (
          ...
          <div className="col-span-12 lg:col-span-3 space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              <div className="flex justify-between items-center mb-3">
                <h4 className="text-sm font-bold text-gray-800">{staticVisaJourneyData.sidebar.summary.title}</h4>
              </div>
              <div className="space-y-1.5 text-xs">
                {staticVisaJourneyData.sidebar.summary.fields.map((field, idx) => (
                  <div key={idx} className={`flex justify-between ${idx !== staticVisaJourneyData.sidebar.summary.fields.length - 1 ? 'border-b border-gray-50 pb-1.5' : ''}`}>
                ...
                {/* Show application ID from API if available */}
                {data.applicationId && (
              ...
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="relative w-28 h-28 mx-auto mb-2">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-bold text-gray-700">{currentStepData.progress || 15}%</span>
            ...
            {currentStep === 2 && staticVisaJourneyData.steps[1].statusTimeline && (
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                <h4 className="text-sm font-bold text-gray-800 mb-3">APS Approval Status</h4>
                <div className="relative pl-4">
                  {staticVisaJourneyData.steps[1].statusTimeline.map((item, idx) => (
            ...
            {currentStep === 3 && staticVisaJourneyData.steps[2].progressSteps && (
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                <h4 className="text-sm font-bold text-gray-800 mb-3">Application Progress</h4>
                <div className="space-y-1">
                  {staticVisaJourneyData.steps[2].progressSteps.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 py-1.5">
            ...
            {currentStep === 4 && staticVisaJourneyData.steps[3].importantInfo && (
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                <h4 className="text-sm font-bold text-gray-800 mb-3">Important Information</h4>
                <div className="space-y-2">
                  {staticVisaJourneyData.steps[3].importantInfo.map((text, idx) => (
            ...
            {/* Quick Links */}
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              <h4 className="text-sm font-bold text-gray-800 mb-2">{staticVisaJourneyData.sidebar.quickLinks.title}</h4>
              <div className="space-y-0.5">
                {staticVisaJourneyData.sidebar.quickLinks.items.map((item, idx) => (
            ...
            {/* Counselor */}
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              <h4 className="text-sm font-bold text-gray-800 mb-3">{staticVisaJourneyData.sidebar.counselor.title}</h4>
              <div className="flex items-center gap-3 mb-3">
                <img src={staticVisaJourneyData.sidebar.counselor.avatar} alt="Counselor" className="w-10 h-10 rounded-full" />
                <div>
                  <h5 className="text-xs font-bold text-gray-800">{staticVisaJourneyData.sidebar.counselor.name}</h5>
                  <p className="text-[10px] text-gray-500">{staticVisaJourneyData.sidebar.counselor.role}</p>
                  <div className="flex items-center gap-1 mt-0.5">
                    <div className="flex -space-x-1">
                      {[1,2,3].map(i => <div key={i} className="w-2 h-2 rounded-full bg-yellow-400 border border-white"></div>)}
                    </div>
                    <span className="text-[8px] text-gray-400">{staticVisaJourneyData.sidebar.counselor.rating} ({staticVisaJourneyData.sidebar.counselor.students} students)</span>
                  </div>
                </div>
              </div>
              ...
              <button className="w-full bg-[#f56e45] hover:bg-[#f56e45] text-white text-xs font-bold py-2 rounded-lg transition-colors">{staticVisaJourneyData.sidebar.counselor.actionButton}</button>
            </div>
          </div>
        </div>
      </main>
