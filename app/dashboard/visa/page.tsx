  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div>
        ...
        <p className={`text-base font-medium ${item.isHighlight ? 'text-green-600' : 'text-gray-800'}`}>
...
const DocumentRow = ({ row, showSize = true }) => (
  <div className="flex flex-col sm:flex-row justify-between items-center py-2 border-b border-gray-50 last:border-0 gap-1">
    ...
    <div className="flex items-center gap-3 sm:gap-4 text-base">
      <span className={`px-2 py-0.5 rounded ${row.status === 'uploaded' || row.status === 'Completed' || row.status === 'Approved' || row.status === 'Verified'
...
const QuickLinkItem = ({ item }) => {
  ...
  return (
    <div className="flex items-center justify-between py-1.5 cursor-pointer hover:bg-orange-50 transition-all duration-300 hover:-translate-y-2 px-2 rounded">
...
const Step1APSApplied = ({ data, currentStepId, apiData, visaId }) => {
  ...
  return (
    <>
      <div className="bg-white p-6 border border-gray-400 -sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
          {(stepData?.sections?.overview?.details || []).map((detail, idx) => (
            <div key={idx}>
              <p className="text-base text-gray-500">{detail.label}</p>
              <p className={`text-base font-bold ${detail.highlight ? 'text-black' : 'text-black'}`}>
                {detail.value || "--"}
              </p>
            </div>
          ))}
      ...
      <div className="bg-white border border-gray-400 rounded-xl overflow-hidden -sm">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              ...
              return (
                <div key={idx} className={`group ${card.bg} border border-gray-400 rounded-xl p-5 hover:border-orange-300 hover:-md transition-all duration-200`}>
          ...
          <div className="px-6 py-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {(stepData?.importantInfo || []).map((item, idx) => (
                <div key={idx} className="bg-white border border-gray-400 rounded-xl p-4">
                  <p className="text-xs uppercase tracking-wide text-gray-400 mb-2">{item.title}</p>
                  <p className="text-base font-medium text-gray-800 break-words">{item.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white p-6 border border-gray-400 -sm">
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="text-[#f56e45] text-base font-bold bg-orange-50 px-3 py-1 rounded" onClick={() => router.push('/dashboard/settings')}>
            View All Documents →
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
          {(apiData?.documents || []).map((item, idx) => {
            ...
            return (
              <div key={idx} className="bg-white border border-gray-400 rounded-xl p-4 mb-4">
                ...
                <div className="mt-4">
                  {!isUploaded ? (
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleViewDocument(item.documentType)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-green-50 border border-green-300 text-base text-green-600 hover:bg-green-100 transition">
        ...
        <div className='md:flex items-center justify-center'>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-4 gap-x-3">
            {(stepData?.sections?.overview?.details || []).map((item, idx) => (
              <div key={idx}>
                <p className="text-base text-gray-500">{item.label}</p>
                <p className={`text-base font-medium ${item.highlight ? 'text-[#f56e45]' : 'text-gray-800'}`}>
                  {item.value || "--"}
                </p>
              </div>
            ))}
          ...
          <div className="mt-6 md:mt-1 flex justify-center">
            <div className="bg-white border border-green-200 p-4 w-full min-w-[180px] flex items-center gap-4 -sm">
              <div className="flex-1">
                ...
                <h5 className="font-bold text-base text-gray-800">APS Certificate</h5>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-gray-100">
              <th className="pb-2 text-base font-semibold text-gray-500">#</th>
              <th className="pb-2 text-base font-semibold text-gray-500">Details</th>
            </tr>
          </thead>
          <tbody>
            {(stepData?.importantInfo || []).map((row, idx) => (
              <tr key={idx} className="border-b border-gray-50">
                <td className="py-2 text-base text-gray-700">{row.title}</td>
                <td className="py-2"><span className="text-base  px-2 py-0.5 rounded">{row.description}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {(stepData?.progressSteps && stepData.progressSteps.length > 0) && (
        <div className="bg-white p-6 border border-gray-400 -sm">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="pb-2 text-base font-semibold text-gray-500">Step</th>
                <th className="pb-2 text-base font-semibold text-gray-500">Status</th>
                <th className="pb-2 text-base font-semibold text-gray-500">Date</th>
                <th className="pb-2 text-base font-semibold text-gray-500">Description</th>
              </tr>
            </thead>
            <tbody>
              {stepData.progressSteps.map((row, idx) => (
                <tr key={idx} className="border-b border-gray-50">
                  <td className="py-2 text-base text-gray-700">{row.label}</td>
                  <td className="py-2">
                    <span className={`text-base px-2 py-0.5 rounded ${row.status === 'completed' ? 'text-green-600 bg-green-50' :
                  ...
                  <td className="py-2 text-base text-gray-500">{row.date}</td>
                  <td className="py-2 text-base text-gray-500">{row.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {stepData.statusTimeline.map((row, idx) => (
                <tr key={idx} className="border-b border-gray-50">
                  <td className="py-2 text-base text-gray-700">{row.date}</td>
                  <td className="py-2"><span className="text-base text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{row.status}</span></td>
                  <td className="py-2 text-base text-gray-500">{row.description}</td>
                </tr>
              ))}
            </tbody>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="pb-2 text-base font-semibold text-gray-500">Status</th>
                <th className="pb-2 text-base font-semibold text-gray-500">Remarks</th>
              </tr>
            </thead>
            <tbody>
              {(apiData?.documents || []).map((row, idx) => (
                <tr key={idx} className="border-b border-gray-50">
                  <td className="py-2 text-base text-gray-700">{row.documentType}</td>
                  <td className="py-2"><span className={`text-base px-2 py-0.5 rounded ${row.status === 'uploaded' ? 'text-green-600 bg-green-50' : 'text-orange-500 bg-orange-50'
                    }`}>{row.status}</span></td>
                  <td className="py-2 text-base text-gray-500">{row.description || "--"}</td>
                </tr>
              ))}
    ...
    <>
      ...
      <div className="bg-white p-6 border border-gray-400 -sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="border border-gray-100 p-4">
            <h5 className="text-base font-bold text-gray-700 mb-3">Father's Details</h5>
            <div className="space-y-2">
              <div><span className="text-base text-gray-500">Name</span><p className="font-medium text-base">{apiData?.user?.familyDetails?.fatherName || '--'}</p></div>
              <div><span className="text-base text-gray-500">Occupation</span><p className="font-medium text-base">{apiData?.user?.familyDetails?.fatherOccupation || '--'}</p></div>
              <div><span className="text-base text-gray-500">Phone</span><p className="font-medium text-base">{apiData?.user?.familyDetails?.fatherPhone || '--'}</p></div>
            </div>
          </div>
          <div className="border border-gray-100 p-4">
            <h5 className="text-base font-bold text-gray-700 mb-3">Mother's Details</h5>
            <div className="space-y-2">
              <div><span className="text-base text-gray-500">Name</span><p className="font-medium text-base">{apiData?.user?.familyDetails?.motherName || '--'}</p></div>
              <div><span className="text-base text-gray-500">Occupation</span><p className="font-medium text-base">{apiData?.user?.familyDetails?.motherOccupation || '--'}</p></div>
              <div><span className="text-base text-gray-500">Phone</span><p className="font-medium text-base">{apiData?.user?.familyDetails?.motherPhone || '--'}</p></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 border border-gray-400 -sm">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-base text-[#f56e45] cursor-pointer hover:underline">View All</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="pb-2 text-base font-semibold text-gray-500 text-center">Updated On</th>
                ...
                return (
                  <tr key={idx} className="border-b border-gray-50">
                    <td className="py-2"><div className="flex items-center gap-2">
                      <FileText size={12} className="text-gray-500" /><span className="text-base text-gray-700">{row.documentType}</span></div></td>
                    <td className="py-2 text-center"><span className={`text-base ${statusColor} px-2 py-0.5 rounded`}>{row.status}</span></td>
                    <td className="py-2 text-center text-base text-gray-400">{row.uploadedAt ? new Date(row.uploadedAt).toLocaleDateString() : '--'}</td>
...
const Step4Biometrics = ({ data, currentStepId, apiData }) => {
  ...
  return (
    <>
      <div className="bg-white p-6 border border-orange-400 -sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-black text-lg ">{stepData?.sections?.overview?.title || "Appointment Details"}</h3>
          <span className="text-base text-black">Updated: {stepData?.sections?.overview?.updated || "Just now"}</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {(stepData?.sections?.overview?.details || []).map((item, idx) => (
  ...
  <div className="flex justify-between items-center mb-4">
    <h3 className="font-bold text-black text-lg">Fees Details</h3>
  </div>
  ...
  <div className="border border-gray-200">
    {(stepData?.importantInfo || []).map((item, idx) => (
      <div
        key={idx}
        className="grid grid-cols-[170px_270px] border-b border-gray-200 last:border-b-0"
      >
        <div className="px-4 font-bold text-black text-base border-r border-gray-200 flex items-center">
          {item.title}
        </div>

        <div className="px-4 py-1 text-base text-gray-800 leading-relaxed line-clamp-4">
          {item.description}
        </div>
      </div>
    ))}
    ...
    {(stepData?.statusTimeline || []).map((item, idx) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          {[
            ...
            <div key={idx} className="flex items-center gap-2 py-1">
              <div className="p-1 bg-orange-50 rounded-full"><CheckCircle2 size={25} className="text-orange-500" /></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div className="bg-white p-6 border border-gray-400 -sm">
          ...
          <p className="text-base text-gray-700 ">Note: Please carry all original documents for verification</p>
        </div>
        <div className="bg-white p-6 border border-gray-400 -sm">
      ...
      <div className="bg-white p-6 border border-gray-400 -sm">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <a href={fileBaseurl(bannerData.fileUrl)} target='_blank' className="border border-orange-400 text-orange-600 text-base font-medium px-4 py-2 rounded hover:bg-orange-50" >Save & Print</a>
        </div>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="bg-white p-6 border border-gray-400 -sm">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 bg-orange-50 p-4 border border-orange-100 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-base text-gray-600 mt-1">Your application is currently under review by the embassy. This may take 4-6 weeks.</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              return (
                <div key={idx} className="flex gap-3">
                  ...
                  <div className="pb-4 flex-1">
                    <div className="flex items-center gap-2">
                      <p className={`text-base font-bold ${isActive ? 'text-[#f56e45]' : isCompleted ? 'text-green-600' : 'text-gray-500'}`}>{item.title}</p>
                    </div>
        ...
        <div className="bg-white p-6 border border-gray-400 -sm">
          ...
          <div className="mt-4 p-3 bg-orange-50 border border-orange-100">
            <h5 className="text-base font-bold text-orange-800 flex items-center gap-1">
            ...
            <ul className="mt-1 space-y-1">
              {[
                ...
                <li key={idx} className="text-base text-orange-700 flex items-start gap-1">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="pb-2 text-base font-semibold text-gray-500">Document Name</th>
                  <th className="pb-2 text-base font-semibold text-gray-500">Status</th>
                  <th className="pb-2 text-base font-semibold text-gray-500">Updated On</th>
                </tr>
              </thead>
              <tbody>
                {(apiData?.documents || []).map((row, idx) => (
                  <tr key={idx} className="border-b border-gray-50">
                    <td className="py-2 text-base text-gray-700">{row.documentType}</td>
                    <td className="py-2"><span className={`text-base ${row.status === 'uploaded' ? 'text-green-600 bg-green-50' : 'text-orange-500 bg-orange-50'} px-2 py-0.5 rounded`}>{row.status}</span></td>
                    <td className="py-2 text-base text-gray-400">{row.uploadedAt ? new Date(row.uploadedAt).toLocaleDateString() : '--'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-1">
            {(stepData?.statusTimeline || []).map((item, idx) => (
              <div key={idx} className="flex gap-3 py-2 border-b border-gray-50 last:border-0">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-base text-gray-400">{item.date}</span>
                    <span className="text-base text-[#f56e45] font-medium">{item.status}</span>
                  </div>
      ...
      <div className="bg-white p-6 border border-gray-400 -sm">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <a href={fileBaseurl(bannerData.fileUrl)} target='_blank' className="border border-gray-400 text-gray-600 text-base font-medium px-4 py-2 rounded hover:bg-gray-50" >Save & Print</a>
        </div>
      </div>
    </>
  );
...
const Step6VisaApproved = ({ data, currentStepId, apiData }) => {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 border border-gray-400 -sm flex flex-col items-center justify-center text-center">
          ...
          <p className="text-base text-gray-600 mt-2">Your Student Visa has been approved. You can now plan your travel.</p>
        </div>
        <div className="bg-white p-6 border border-gray-400 -sm">
            ...
            {[
              ...
              return (
                <div key={idx} className="flex flex-col items-center text-center flex-1 min-w-[80px]">
                  ...
                  <p className="text-base font-bold text-gray-700">{step.title}</p>
                </div>
              );
            })}
          </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {(apiData?.documents || []).map((row, idx) => (
                <tr key={idx} className="border-b border-gray-50">
                  <td className="py-2 text-base text-gray-700">{row.documentType}</td>
                  <td className="py-2"><span className={`text-base ${row.status === 'uploaded' ? 'text-green-600 bg-green-50' : 'text-blue-600 bg-blue-50'} px-2 py-0.5 rounded`}>{row.status}</span></td>
                  <td className="py-2 text-base text-gray-400">{row.uploadedAt ? new Date(row.uploadedAt).toLocaleDateString() : '--'}</td>
                </tr>
              ))}
            </tbody>
    ...
    return (
      <div className="space-y-6">
        <div className="bg-green-50 p-6 border border-green-200 text-center">
          ...
          <button className="mt-4 bg-green-600 hover:bg-green-700 text-white text-base font-medium px-4 py-2 transition-colors">
            Go to Next Step
          </button>
        </div>
      </div>
            ...
            <tbody>
              {[
                ...
                <tr key={idx} className="border-b border-gray-50">
                  ...
                  <td className="py-2 text-center"><span className="text-base text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded">{row.status}</span></td>
                  <td className="py-2 text-right">
      ...
      <main className="max-w-[1600px] mx-auto p-4">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
            {pageData.title}
            <span className={`text-base font-normal px-2 py-0.5 rounded ${pageData.status === 'Approved' ? 'bg-green-100 text-green-700' :
        ...
        {progressSteps.length > 0 && (
          <div className="bg-white p-6 border border-gray-400 -sm mb-6 overflow-x-auto">
        ...
        {bannerData.title && (
          <div className={`${getBannerStyles()} relative overflow-hidden rounded-xl p-5 mb-6 -sm transition-all duration-300 ${bannerData?.type === 'success'
                ...
                {bannerData?.progress !== undefined && (
                  <div className="mt-3">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${bannerData?.type === 'success' ? 'bg-green-500' : 'bg-orange-500'
                          }`}
          ...
          <div className="col-span-12 lg:col-span-3 space-y-6">
            <div className="bg-white p-5 border border-gray-400 w-full max-w-sm">
              ...
              <div className="space-y-4 ">
                ...
                <div className="grid grid-cols-[130px_1fr] gap-4 border-b pb-3">
                  <span className="text-black font-medium text-base">Student Email</span>
              ...
              <div className="w-full space-y-1 mt-1">
                {progressSteps.map((step, idx) => (
                  <div key={idx} className="flex justify-between items-center border-b border-gray-50 pb-1">
                    <div className="flex items-center gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full ${step.page?.status === 'Completed' || step.status === 'Completed' ? 'bg-green-500' :
            ...
            <div className="bg-white p-4 border border-gray-400 -sm">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h5 className="text-base font-bold text-gray-800">Sarah Johnson</h5>
                  <p className="text-base text-gray-500">Senior Visa Counselor</p>
                  <div className="flex items-center gap-1 mt-0.5">
                    <div className="flex -space-x-1">
                      {[1, 2, 3, 4, 5].map(i => <div key={i} className="w-2 h-2 rounded-full bg-yellow-400 border border-white"></div>)}
                    </div>
                    <span className="text-base text-gray-400">4.9 (128 students)</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-2 mb-3">
                <button className="flex-1 p-1.5 border border-orange-400  flex justify-center"><MessageCircle size={14} className="text-orange-500" /></button>
                <button className="flex-1 p-1.5 border border-orange-400  flex justify-center"><Phone size={14} className="text-orange-500" /></button>
                <button className="flex-1 p-1.5 border border-orange-400  flex justify-center"><Mail size={14} className="text-orange-500" /></button>
              </div>
