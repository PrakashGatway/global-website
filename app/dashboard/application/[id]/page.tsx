...
export default function StudentDetailsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "information" | "documents" | "activity"
  >("information");
  ...
  const fetchActivities = async () => {
    try {
      const response = await axiosInstance.get(
        `/communication/applications/${application._id}/activities?limit=100`,
      );
  ...
  const handleSubmitAnswer = async (e: React.FormEvent) => {
    ...
    try {
      fields =
        typeof selectedRequirement.extra === "string"
    ...
    try {
      if (uploadedFiles.length > 0) {
        ...
        const result = await uploadDocument(
          ...
          (progress) =>
            setUploadProgress((prev) => ({
              ...prev,
              [selectedRequirement._id]: progress,
            })),
        );
        if (result.success) {
        ...
        const response = await axiosInstance.put(
          ...
          {
            answer: JSON.stringify(dynamicValues),
  ...
  const timelineSteps = getApplicationFlow(application?.primaryStatus).map(
            ...
            <div className="flex items-top gap-3">
              ...
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  <Link
                    href={`/dashboard/programs/${application?.course?.slug}`}
                    className="flex  items-center gap-1"
                  >
                    <h1 className="text-2xl font-medium text-gray-800 hover:text-orange-900">
                  ...
                  {application.course?.applicationFee > 0 &&
                    application?.paymentStatus == "Pending" && (
                      <span
                        className={`px-3 py-1 text-xs font-medium border flex items-center gap-1.5 ${getPaymentStatusBadge(application?.paymentStatus)}`}
                      >
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="inline-flex">
                    <Link
                      href={`/dashboard/universities/${application?.course?.university?.slug}`}
                      className="text-orange-500 underline"
                    >
                ...
                <div className="flex items-center text-gray-700 text-sm gap-2 mt-1 flex-wrap">
                  ...
                  <button
                    ...
                    className="inline-flex hover:text-orange-500 p-1 transition-colors"
                  >
                    <Edit2Icon className="w-4 h-4" />
                  </button>{" "}
                  |
                  <span>
                    Submission deadline: {application?.deadline || "N/A"}
                  </span>
                </div>
              </div>
            </div>
  ...
  `}
                        >
                          {step.icon ? (
                            <step.icon
                              className={`w-5 h-5  ${step.status === "completed" ? "text-white" : "text-gray-500"}`}
                            />
                          ) : (
                        ...
                        {step.status && (
                          <p
                            className={`text-xs  mt-1 ${step.status === "completed" ? "text-black" : "text-gray-500"}`}
                          >
                            {step.status}
                          </p>
                        )}
          ...
          <div className="bg-white w-full max-w-2xl rounded-xl shadow-lg p-6">
            ...
            <div className="min-h-[200px]">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-6 overflow-y-auto max-h-[400px]">
                {(application?.course?.university?.intakes || []).map(
                  (item: string) => (
                    <div
                      key={item}
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <div
                          className={`w-4 h-4 rounded-full border-2 ${selectedIntake === item ? "border-orange-500 bg-orange-500" : "border-gray-300"}`}
                        >
                          {selectedIntake === item && (
            ...
            >
              ...
              >
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          selectedRequirement.required === "required"
                  ...
                  {selectedRequirement.docType == "form" ? (
                    ...
                    <>
                      ...
                      <div className="mb-4">
                        ...
                        {uploadProgress[selectedRequirement?.id] > 0 &&
                          uploadProgress[selectedRequirement?.id] < 100 && (
                            <div className="mt-4">
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <span className="text-xs text-gray-500 font-medium">
                                  {uploadProgress[selectedRequirement.id]}%
                                </span>
                              </div>
                              <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                                <motion.div
                                  initial={{ width: 0 }}
                                  animate={{
                                    width: `${uploadProgress[selectedRequirement.id]}%`,
                                  }}
                ...
                <div className="p-4 border-t bg-white flex gap-3 shrink-0">
                  ...
                  <button
                    type="submit"
                    className="flex-1 px-4 py-2 bg-[#ff6a1a] text-white rounded-lg hover:bg-[#f45f0d] transition-colors disabled:opacity-50"
